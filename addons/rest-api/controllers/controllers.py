...
def decode(encoded_str):
    key = base64.urlsafe_b64encode(bytes("mOJq9EUyB62k5twlcLdAWnBAqb7pMwwh", 'utf-8'))
    fernet = Fernet(key)
    decMessage = fernet.decrypt(bytes(encoded_str, 'utf-8')).decode()
...
class OdooAPI(http.Controller):
    ...
    def authenticate(self, *args, **post):
        ...
        http.request.session.authenticate(db, login, decode(password))
        res = request.env['ir.http'].session_info()
        return res

    @http.route(
        '/extend-object/v1/<string:model>/<string:function>',
    ...
    @http.route(
        '/extend-object/v1/<string:model>/<int:rec_id>/<string:function>',
        type='json', auth='user', methods=["POST"], csrf=False)
    ...
    @http.route(
        '/extend-api/v1/<string:model>/<int:rec_id>',
        type='http', auth='user', methods=['GET'], csrf=False)
    ...
    @http.route(
        '/extend-api/v1/<string:model>/<int:rec_id>/<string:field>',
        type='http', auth="user", methods=['GET'], csrf=False)
